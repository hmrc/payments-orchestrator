...
object ValueClassBinder: 
  ...
  def bindableA[A: Tag: Reads](fromAtoString: A => String): QueryStringBindable[A] = new QueryStringBindable.Parsing[A](
    parse = JsString(_).as[A],
    fromAtoString,
    {
      case (key: String, _: Exception) => s"Cannot parse param $key as ${summon[Tag[A]].tag.toString}"
    }
  )
